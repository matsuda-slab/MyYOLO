...
def build_targets(p, targets, model):
    ...
    for i, yolo_layer in enumerate(model.yolo_layers):
        anchors_tmp = yolo_layer.anchors
        anchors_tmp = torch.tensor(list(chain(*anchors_tmp))).to(targets.device).float().view(-1, 2)
        anchors = anchors_tmp / yolo_layer.stride
